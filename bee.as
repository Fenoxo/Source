//PLACEHOLDER EVENTS ARE 9999 - search and destroy!
...
function beeEncounter():void {
	...
	if(player.hasKeyItem("Traveler's Guide") >= 0 && player.inte/2 > rand(40)) {
		...
		simpleChoices("Yes", 2052, "No", 0, "", 0, "", 0, "Back", 13);
	}
	...
	if(player.isTaur()) {
		...
		//Route 1 
			...
			if(player.biggestTitSize() > 1) {
				outputText("  You begin pounding away at your " + allBreastsDescript() + " in an attempt to alleviate the pressure in your inaccessible groin.", false);
				//[Fuckable breasts:
...
function rapeTheBeeGirl():void {
	...
	//OPTIONS HERE!
	var naga:Number = 0;
	var multiCock:Number = 0;
	var cock:Number = 0;
	var vagina:Number = 0;
	var herm:Number = 0;
	var gentleman:Number = 0;
	var eggs:int = 0;
	if(player.canOvipositSpider() && (player.faceType == 5 || player.faceType == 10)) {
		eggs = 3833;
	...
	if(player.isNaga() && player.faceType == 5) {
		outputText("(You could focus on your snakelike, 'naga' attributes.)\n", false);
		naga = 2777;
	}
	if(player.cor >= 75 && player.str >= 60 && (player.tongueType == 1 || player.hasCock() || player.hasVagina() || player.biggestTitSize() >= 4)) {
...
function beeGirlRapeForTheDistinguishedGentleman():void {
	...
	if(player.tongueType == 1) choices[choices.length] = 0;
	...
	//[(if dick)
	if(player.hasCock()) outputText("stroke your " + cockNoun(0), false);
...
function beeAlternate():void {
	...
	if(rand(2) == 0) {
		...
		if(player.cocks[x].cockType == 1) outputText("You loosen your " + player.armorName + " and unleash your cocks.  The equine flare of the largest is already drooling pre as you enjoy the sight of the helpless, squirming bee-girl.  Swinging it like a flogger, you spank her ass, making the girl yelp and leaving trails of sticky pre across the globes of her ass.  Grinning, you drag your shaft back and forth across her pussy, slicking it in her honey.  Finally, you press the tip against her opening.  She's a tight fit for you, and she squeals more loudly as your shaft spreads her little cunny inexorably.  ", false);
		//Catchall scenario
		if(player.cocks[x].cockType == 3 || player.cocks[x].cockType > 4) outputText("You loosen your " + player.armorName + " and unleash your cocks.  The glans of the largest is already drooling pre as you enjoy the sight of the helpless, squirming bee-girl.  Guiding it forwards, you spank her ass, making the girl yelp and leaving trails of sticky pre across the globes of her ass.  Grinning, you drag your shaft back and forth across her pussy, slicking it in her honey.  Finally, you press the tip against her opening.  She's a tight fit for you, and she squeals more loudly as your shaft spreads her little cunny inexorably.  ", false);
		//CONTINUE
		...
		if(player.cocks.length >= 3) {
			outputText("  Her battered " + eVaginaDescript(0) + " grips down on your dual " + cockDescript(99) + "s tightly, draining you of every drop she can.  Rivers of white begin to leak from her abused holes and mingle with her own release as it trails down her abdomen and drips off it slowly onto the forest floor.", false);
		...
		if(player.lowerBody != 4) outputText("stuffing your " + multiCockDescriptLight() + " inside your " + player.armorName, false);
...
function nagaRapesPt2TheExtremeContinuationOfAwesome():void {
	...
	//[Player is female]
		...
		if(player.vaginas[0].vaginalWetness < 3) outputText("A few drops ", false);
		else if(player.vaginas[0].vaginalWetness < 4) outputText("A gush ", false);
		else if(player.vaginas[0].vaginalWetness < 5) outputText("Wave after wave ", false);
