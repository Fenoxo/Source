...
function encounterMinerva():void {
	if(flags[MET_MINERVA] == 0) firstMinervaEncounter();
	else if(flags[MET_MINERVA] == 3) minervaThirdPlusEncounter();
	else if(flags[MINERVA_BACKSTORY_LEARNED] > 0 && player.gender > 0 && flags[MINERVA_LOVE] == 0 && rand(2) == 0) minervaAfterFiveTalksRomancing();
	else {
...
function minervaTalkSelect(bath:Boolean = true):void {
	...
	if(flags[MINERVA_BACKSTORY_LEARNED] == 0) {
		//Force first time to talk about her early backstory!
		if(flags[MINERVA_BACKSTORY] == 0) {
	...
	if(minervaRomanced() && flags[MINERVA_BACKSTORY_LEARNED] == 0) choices[choices.length] =  talkWithMinervaAboutSharkGirlDaughter;
...
function talkingToMinervaAboutBackstory():void {
	...
	//PART 2
	else if(flags[MINERVA_BACKSTORY] == 1) {
		flags[MINERVA_BACKSTORY] = 2;
	...
	//PART 3
		...
		// PC returns to camp.
		flags[MINERVA_BACKSTORY_LEARNED] = 1;
	}
	doNext(13);
}
...
function talkWithMinervaAboutSharkGirlDaughter():void {
	...
	//- requires that backstory has been told
	//- if already romanced Minerva shamefully confesses she may have fathered a granddaughter with her first daughter -repeatable
	outputText("Telling Minerva that you want to sit down and talk brings a pleasant smile to her face. The siren more than happy to talk with you.  \"<i>Well, since you came all this way to see mem I guess we could talk</i>\" she says playfully.  \"<i>All right, what do you want to know?</i>\" she asks as she sits down cross-legged on the soft, pillowy moss across from you.");
...
function bathTimeWithMinerva():void {
	...
	outputText("\n\nLooking back at you, Mineva gasps and lets go of your hand, obviously quite embarrassed by her actions.  \"<i>Oh, [name], I'm so sorry!  I guess I got a little carried away, didn't I?</i>\" she asks as she looks at you, her head turned down and to the side in her self-inflicted humiliation.  Letting out a sigh, you shake your head and smile before slowly removing your [armor] and placing your garments on the mossy side of the spring to dry.  As if mesmerized by you, Minerva watches your undress, her eyes glued to your form and trailing up and down your body, a dopey, lovestruck look on her face.");
	...
	outputText("\n\nGrinning at her work, Minerva cuddles up against you in the now steaming water, and you certainly can't say you're not impressed with that little trick of hers.  Letting out a small moan approval, both of you lay back and rest in the temporarily hot spring, just relaxing peacefully together, a simple affectionate pastime shared between two lovers.  Minerva holds onto you gently, and her bare breasts presses against your arm as one of her hands tenderly stroke your form, drawing a relaxed sigh from you.");
...
function minervaSexMenu(display:Boolean = true):void {
	if(display) {
		...
		//Default, first time:
		if(flags[TIMES_MINERVA_SEXED] == 0) {
...
function fuckMinervasAsshole():void {
	...
	if(!minervaRomanced()) {
		//First time ending:
		if(flags[TIMES_BUTTFUCKED_MINERVA] == 0) {
	...
	flags[TIMES_BUTTFUCKED_MINERVA]++;
...
function minervaCowgirlSex():void {
	...
	//First time:
	if(flags[TIMES_MINERVA_COWGIRLED] == 0) {
...
function minervaLapSex():void {
	...
	if(!minervaRomanced()) {
		//Default, first time:
		if(flags[TIMES_MINERVA_LAPSEXED] == 0) {
			outputText("\n\nMinerva actually seems a little sad that you're leaving.  \"<i>I guess you are a busy person.  But I'm glad we did this, it was a lot of fun.  And... you know...  I wouldn't mind if we did this again, soon,</i>\" She suggests with a small, hopeful smile on her face.  You promise her that you'll try to come back again soon.");
		}
		//Default, repeated:
		else {
...
function noBallsMinervaAndContinuation(clear:Boolean = true):void {
	...
	//if demonic tongue:
	if(player.tongueType == 2) outputText(", your perversely prehensile tongue slowly retracting from her quivering quim, rubbing and teasing as it comes out and nearly sending the overly sensitive herm into another climax");
	outputText(".");
...
//Flying kick:
function minervaKnowsKungfu():void {
...
function tailWhip():void {
	...
	//[if attack lands]
    	...
    	damage = takeDamage(damage);
		outputText(" (" + damage + ")");
		if(monster.hasStatusAffect("Tail Whip") >= 0) monster.addStatusValue("Tail Whip",1,10);
		else monster.createStatusAffect("Tail Whip",10,0,0,0);
	}
	combatRoundOver();
}

//Halberd stab:
function minervaUsesHalberdStab():void {
...
//Halberd CHOP:
function minervaUsesHalberdCHOP():void {
...
//Booty-shorts
function bootyShortInYoFaceSon():void {
	...
	stats(0,0,0,0,0,0,20+player.lib/10+rand(5),0);
	combatRoundOver();
}

//Lust Attacks for all Minervas
//Pole licking
function lickDatPole():void {
...
function sirensSong():void {
	...
	//[part 2]
		...
		//If wait: 
		if(flags[22] == 1) outputText("  You cover your ears before she even opens her lips, wary of its power.  Judging by the dim feeling of pleasure simmering through you with the little sound you're picking up regardless, it probably was for the better.");
		//No wait - insta loss:
		else {
		...
		monster.removeStatusAffect("Siren Song");
	}
	combatRoundOver();
}

function minervaAI():void {
	if(monster.hasStatusAffect("Siren Song") >= 0) sirensSong();
	else if(rand(25) == 0 || (monster.HP < 100 && rand(2) == 0)) sirensSong();
	//Else choose randomly!
	else {
		var choices:Array = new Array();
		choices[choices.length] = lickDatPole;
		choices[choices.length] = bootyShortInYoFaceSon;
		choices[choices.length] = kiteFire;
		choices[choices.length] = minervaUsesHalberdCHOP;
		choices[choices.length] = minervaUsesHalberdStab;
		choices[choices.length] = tailWhip;
		choices[choices.length] = minervaKnowsKungfu;
		choices[choices.length] = minervaBite;
		choices[rand(choices.length)]();
	}
}

function loseToMinerva():void {
	clearOutput();
	//PC loss by physical
	if(player.HP < 1) {
...
function beatUpDatSharpie():void {
	clearOutput();
	//Win by damage
	if(monster.HP < 1) {
...
function getADrinkYouBitch():void {
	...
	stats(0,0,0,0,0,0,-25,-.5,false);
	if(player.cor > 50) stats(0,0,0,0,0,0,0,-1);
	if(player.cor > 75) stats(0,0,0,0,0,0,0,-1);
	doNext(13);
}
/*
