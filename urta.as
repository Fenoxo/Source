//Urta variables 
//flags[11] = times fucked
//flags[12] = urtas horsecock comfort level	
	//-1 = PC pissed her off
	//0 = first timern
	//1-10 = slightly comfortable
	//11+ = flaunt it
//flags[13] = hours turned off
	//0 = ready to rock!
	
//Urtas horsey bits =
	//LxW = 20x3
//Luv Lock - flags[30]:
//0 = normal, 1 = love, -1 = never love, flags[12] -1 = hate
/*Luv rating! - flags[29]
...
function urtaLove(love:Number = 0):Boolean {
	trace("URTA HORSECOCK COMFORT LEVEL: " + flags[12]);
	if(flags[30] == -1) return false;
	if(love == 0) {
		if(flags[30] == 1) return true;
		else return false;
	}
	if(love > 0) {
		flags[29] += love;
	...
	if(flags[30] == 1) {
		...
		if(love > 0 && flags[AMILY_VISITING_URTA] == 0 && amilyFollower() && flags[43] == 1) flags[AMILY_NEED_TO_FREAK_ABOUT_URTA] = 1;
		return true;
	}
	else return false;
}
function urtaDrunk():Boolean {
	//Preg = no drinking!
	if(flags[URTA_EGG_INCUBATION] > 0) return false;
	if(flags[URTA_INCUBATION] > 0) return false;
	if(flags[146] == -1) return false;
	if(hours > 12 && flags[146] < 1) return true;
	if(hours > 8 && flags[146] >= 1) return true;
	else return false;
}
function urtaOpenAboutEdryn():Boolean {
	//Did it come out from the scylla threesome?
	if(flags[145] > 0) return true;
	//Did it come out from marble?
	if(flags[32] > 0) return true;
	//Did it come out from discussions?
	if(flags[URTA_OPEN_ABOUT_EDRYN] > 0) return true;
	return false;
}


function urtaBarDescript():Boolean {
	outputText("\n\n", false);
	//Urta PISSED
	if(flags[31] > 1) {
	...
	//[Post Pissed Talk]
	if(flags[31] == 1) {
		outputText("Urta is sitting a table, swishing a bottle around and looking forlorn.  She looks up at you and sighs, \"<i>Look, I still don't think we'll ever be the same, but to be frank, we're great in the sack and I've been lonely.  For now, I'll put the past behind me.</i>\"", false);
		flags[31] = 0;
		return true;		
	}
	//Raphael betrayed reward
	if(flags[133] == -1 && flags[148] == 0) {
	...
	if(flags[URTA_EGG_INCUBATION] > 0) {
		//Egg Level 1: 
		if(flags[URTA_EGGS] < 20) outputText("Urta is sitting quietly at her usual seat, sipping a mug of some non-alcoholic beer.  Though her clothes are still in place, you can just make out the bulging midriff stretching them taut, a consequence of all the eggs in her womb.");
		else if(flags[URTA_EGGS] < 40) outputText("Urta is sitting rather uncomfortably at her usual seat.  Her swollen stomach is so large it keeps her from wearing her usual outfit, instead forcing her into a mini-shirt and a skirt bottom, allowing her midriff to be bare.  She occasionally runs a hand over the expanse, as if she can't believe it's actually there.");
		//Egg Level 3: 
	...
	if(flags[URTA_TIMES_EGG_PREGGED] > 0 && flags[URTA_EGG_INCUBATION] == 0 && flags[URTA_FLATBELLY_NOTICE] == 0) {
		...
		flags[URTA_FLATBELLY_NOTICE] = 1;
		return true;
	}
	//[Love Urta Bar Appearance]
	if(flags[30] == 1) {
		if(flags[146] == -1 || !urtaDrunk()) outputText("Urta is sitting at her usual table, sipping a glass of wine and wearing a form-fitting evening gown of shimmering black.  She looks up at you, happiness filling her eyes when she notices you entering the bar.", false);
		//(DRUNK) 
	...
	if(flags[11] > 0) {
		...
		if(flags[12] < 11) outputText("Thankfully she still has her other addition trapped under her tail, for now.", false);
...
function urtaBarApproach():void {
	...
	if(flags[145] == 0 && flags[143] > 0) {
		...
		else if(flags[12] >= 12) outputText("anxiously", false);
	...
	//TO ZE FLIPOUT!
	if(flags[URTA_EGG_FORCE_EVENT] > 0) {
		urtaChewsOutPC();
		return;
	}
	//PREGNANT URTA
	...
	if(flags[13] > 0 && !urtaDrunk()) {
		//Cockblock marble convo
		if(player.hasStatusAffect("Camp Marble") >= 0 && flags[32] == 0) {
			flags[32] = 1;
			marbleCockuBlockuUrta();
			return;
		}
		//Twu wuv talk
		if(flags[29] >= 30 && flags[30] == 0) {
			UrtaTwuWuvOffer();
			return;
		}
	...
	if(flags[30] == 1) 
	{
		...
		if(flags[146] == -1 || !urtaDrunk()) {
			...
			//simpleChoices("Her Place",2300,"Suck Off",2354,"Eat Out",2350,"",0,"",0);
			menu();
	...
	if(flags[12] >= 11 && rand(2) == 0 ) {
		...
		if(flags[12] < 11) outputText("moments before you hear her horse-cock thump into the bottom of the table.  ", false);
...
function dualUrtaMasturbation():void {
	...
	if(player.hasVagina()) {
		if(player.vaginas[0].vaginalWetness > 3) outputText("A trickle of wetness escapes your nether-lips, starting to puddle underneath you.", false);
	...
	if(player.hasVagina()) {
		...
		if(player.vaginas[0].vaginalWetness == 5) outputText(" and gushing everywhere", false);
		outputText(".  ", false);
	}
...
function urtaFuckHer():void {
	...
	if(player.totalCocks() == 0) {
		...
		if(player.tailType == 1 || player.tailType == 2) outputText("swishing your tail across her taut nipples", false);
...
function blowUrtaUnderTable():void {
	...
	if(flags[12] >= 11) outputText(" and pulling down her fishnet to release the monster trapped inside it.  It snaps up immediately, bobbing before your face as it fills with Urta's booze-fueled lust.", false);
...
function takeUrtaInTheButtPublically():void {
	...
	urtaLove(2);
	flags[13] = 5;
	if(player.cor < 60 && flags[23] == 0) outputText("You can't believe you're doing it, but you undo the lower half of your " + player.armorName + ".  ", false);
	...
	if(flags[11] < 11) outputText("unwinds her tail from around the beast", false);
	...
	if(player.hasVagina()) {
		...
		outputText(".  ", false);
		if(player.vaginas[0].vaginalWetness < 3) outputText("Moisture beads on your lips, signaling just how turned on this total violation is making you.", false);
		else if(player.vaginas[0].vaginalWetness < 5) outputText("Moisture begins dripping onto Urta's thighs, signaling just how turned on this total violation is making you.", false);
	...
	else if(player.cor < 50 && flags[23] == 0) outputText("you find yourself whimpering, regretting the decision to climb on top of her, but aroused in spite of it.", false);
...
function getAPublicFacialFromUrta():void {
	...
	if(flags[12] < 11) outputText("skirt ", false);
	...
	if(player.cor > 50) {
		...
		if(flags[12] >= 11) outputText("stuffs herself back into her fishnet", false);
...
function goBackToUrtasForLuvinz():void {
	...
	if(flags[30] == 1) {
		...
		if(flags[URTA_PETPLAY_DONE] == 0 && urtaLove()) outputText("  <b>A new, shiny leather collar lies half-concealed under a cushion, as though its owner haphazardly tried to bury it in a rush.</b>");
...
function goBackToUrtasForLuvinzII():void {
	...
	if(flags[14] == 0) {
		...
		if(giacomo > 0) outputText("  You recognize some of the toys as Giacomo's wares, though there are a fair number that seem to be custom made, and others that are clearly living creatures.", false);
	...
	var femaleRides:Number = 0;
	if(player.totalCocks() > 0) vaginal = 2302;
	if(player.hasVagina()) femaleRides = 2301;
	//simpleChoices("Fuck Vagina",vaginal,"Ride (Vaginal)",femaleRides,"Ride (Anal)",2305,"No Condoms",3967,"",0);
	menu();
	if(vaginal > 0) addButton(0,"Fuck Vagina",eventParser,vaginal);
	if(femaleRides > 0) addButton(1,"Ride(Vaginal)",eventParser,femaleRides);
	addButton(2,"Ride(Anal)",eventParser,2305);
	addButton(3,"No Condoms",eventParser,3967);
	if(player.isGoo() && player.skinType == 3) addButton(4,"Goo (Weird)",urtaGooTesticleVoreRuinedOrgasms);
}
...
function rideUrtaTenderFemale():void {
	...
	if(player.vaginas[0].vaginalWetness < 2) outputText("growing moistness in your loins", false);
	else if(player.vaginas[0].vaginalWetness < 4) outputText("sopping wetness in your loins", false);
	...
	if(player.biggestTitSize() >= 5) {
		...
		if(player.totalCocks() > 0) {
			if(player.cocks[0].cockLength > 15) {
				...
				else outputText("  Each of your " + multiCockDescriptLight() + " pushes up between the jiggling breasts, slicking them with your pre-cum as they begins to get tit-fucked by both of your trembling forms.", false);
			}
		}
		outputText("\n\n", false);
	}
	...
	//if(flags[URTA_FERTILE] == 1) player.knockUp(21,515,80);
	stats(0,0,0,0,0,-2,-100,0);
	doNext(13);
}
...
function dudeFuckingUrtasCooch():void {
	...
	outputText("Obligingly, you ease your hips forward, reeling at the heat of her cunt-lips as they welcome your " + cockDescript(0) + "'s " + cockHead(0) + " with slippery moisture.  ", false);
...
function marbleCockuBlockuUrta():void {
	...
	//if nursery
	if(flags[9] == 100) outputText(" and a nursery", false);
	outputText(".", false);
	//(Not wuv'ed yet)
	if(flags[30] == 0) {
...
function TellUrtaDAHTRUUUUF():void {
	...
	if(flags[30] == 1) outputText("I love you for it", false);
	...
	if(!urtaOpenAboutEdryn()) {
		...
		if(flags[30] == 1) outputText("been together", false);
...
function toughenUpUrtaAfterTellingTruuf():void {
	...
	if(flags[30] == 1) outputText("heartbroken", false);
...
function lietoUrtaAboutMarble():void {
	...
	if(flags[8] == 1) outputText(" and saw the kid", false);
	if(flags[9] > 1) outputText(" and saw the kids", false);
...
function blowUrtaUnderTheTableLuv():void {
	...
	if(player.cor < 50 && flags[23] <= 0) outputText("wishing there was a tablecloth to hide your activities from prying eyes", false);
...
function rideUrtasCoochLoveyDovey():void {
	...
	if(player.vaginas[0].vaginalWetness < 2) outputText("growing moist", false);
	else if(player.vaginas[0].vaginalWetness < 4) outputText("growing wetter and wetter as you lubricate the tip of her condom", false);
...
function rideUrtaInButtAtHomeLove():void {
	...
	if(player.hasVagina()) {
		...
		if(player.vaginas[0].vaginalWetness < 4) outputText("dampens", false);
	...
	//(EVERYBODY ELSE)
		...
		if(player.hasVagina()) {
			...
			if(player.vaginas[0].vaginalWetness > 2 && player.vaginas[0].vaginalWetness < 5) outputText("  Fluids soak Urta's crotch with lust with feminine drool, though the fox barely seems to notice.", false);
			else if(player.vaginas[0].vaginalWetness == 5) outputText("  An explosion of feminine fluids spatters Urta's crotch and bed, completely soaking her groin in your need, but the distracted fox barely notices.", false);
		}
		if(player.totalCocks() > 0) {
...
function oralFiestyUberExplosionUrta():void {
	...
	if(player.gender == 2 || (player.gender == 3 && rand(2) == 0)) {
		...
		if(player.vaginas[0].vaginalWetness < 4) outputText("equally wet", false);
		...
		if(player.vaginas[0].vaginalWetness > 4) outputText("Fluids rush from your folds, splattering her face with sex fluid.  The fox opens wide, gulping it down like someone in a desert who has just found an oasis.", false);
...
function urtaHomeLuvLuvinsMenu():void {
	...
	if(player.hasVagina()) {
		...
		addButton(0,"Ride Vag",eventParser,2355);
	}
	...
	if(player.gender > 0) {
		...
		addButton(4,"No Condoms",eventParser,3967);
	}
	if(player.isGoo() && player.skinType == 3) addButton(6,"Goo (Weird)",urtaGooTesticleVoreRuinedOrgasms);
	if(flags[URTA_PETPLAY_DONE] >= 0 && player.gender > 0) addButton(7,"Collar",urtaPetPlayDeletedForeverBecauseThirdProovedMeWrongAboutDice);
	return;
}
...
function urtasCoochiNeedsFuckedLove():void {
	...
	if(player.hasKnot(x)) {
		...
		else if(player.cumQ() < 1000) outputText("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your overproductive " + cockNoun(0) + " stops spewing inside her.  ", false);
		else {
		...
		outputText("You finally feel your " + cockNoun(0) + "'s knot shrinking and pull free with a loud and messy pop.  ", false);
	...
	//[HORSE]
		...
		outputText("  Crotch muscles clench in an unconscious rhythm as orgasm wracks your body.  You begin spurting hard inside her, splattering it into her cervix, even allowing it to flow into her womb.  Urta cries, \"<i>ooooOOOHHHH YESSS....is this what you feel when I cum inside you?  YesYESYES-Ohhh I can feel the flare stroking and flexing inside me...</i>\"\n\n", false);
		...
		else if(player.cumQ() < 1000) outputText("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your over-productive " + cockNoun(0) + " has finished spewing your seed inside her.  ", false);
		else {
...
function scyllaAndUrtaSittingInATree():void {
	...
	if(flags[147] == 0 && flags[145] > 0) {
		...
		else if(flags[12] >= 11) outputText("giving you a lewd look", false);
		...
		else if(flags[12] >= 11) outputText("Ohhh, you showed up a little late... mmmhmm yes, now lick... Oh, why didn't you tell me ho - OH YES!  RIGHT THERE! - h-how good of a cock-sucker this lewd-bodied bitch was?", false);
...
function fuckUrtaWhileScyllaSucksSober():void {
	...
	outputText(" to your " + cockHead(x) + ".  She's moaning so hard and fast that there's barely time for her to gasp in fresh lungfuls of air, and she begins to saw her hips back and forth with reckless abandon, splattering your groin with slick wetness and face-fucking the hungry nun wildly, lost to her passion.");
	...
	outputText("\n\nThe nun jumps up and hugs the exhausted fox tight.  Then, blushing, she quickly changes into a fresh habit and flounces out the door, taking the old, soaked one with her, perhaps for a snack.  Urta grumbles, \"<i>Go on, it's mostly my mess, so I suppose I ought to pay for the clean up.</i>\"  She props herself against a box as her equine member spontaneously gains a few inches, dripping fresh drops of pre.  Her balls seem back to normal too.");
...
function dockWithUrtaUnderScyllasSoberSupervision():void {
	...
	if(flags[URTA_SCYLLA_BIG_DICK_TIMES_DONE] > 0) outputText("Just like last time, she adopts a panicked expression and lunges for the door, but she moves slowly and a gooey drop of pre-cum hangs from her flare as she moves.");
	else outputText("With a panicked expression on her face, she lunges for the door, her hard cock flopping awkwardly.");
	outputText("  Scylla's eyes glow black as she reaches for the guardswoman, but she's too far to even catch the fleeing fox's tail.  An inexplicable black radiance glows from the holy woman-turned-succubus's eyes, and miraculously, Urta is lifted clear of the floor, her legs flailing uselessly.  She flies back to the corner of the room and lands in confused pile, less than a foot from your carriage-sized erection.");
	outputText("\n\nMeanwhile, your erection continues to grow unimpeded, and it now it reaches across the smallish room, easily as tall as your waist and utterly immobilizing.  The magically restrained fox struggles, but semi-transparent tendrils of inky energy keep her seated.  They twist around her ears and turn her face, forcing her to look at your tumescent, oozing penis.  Directly in the center of your giant " + cockHead(x) + " lies your plus-sized cum-slit, as big as a centaur's cunt and stretched wide by a dollop of your fragrant cock cream.  Urta is forced to watch it slowly roll out to puddle on the floor.  Her own erection, infantile by your new standards, rises fitfully, the flare filling to a full erection as Urta's body betrays her, overpowering her fear.");
	...
	outputText("\n\nScylla gestures at your " + cockDescript(x) + " and commands, \"<i>Go on, fuck it.  You know you want to.</i>\"  The black restraints poof into phantasmal smoke.  At the same time, the your lusty hermaphrodite ");
	...
	outputText("\n\nA like a firework, something fires off deep inside you, but the explosion of pleasure that rattles in your skull is nothing to sneeze at.  Your bliss seems to have a kinetic force that hits you hard enough to cross your eyes.  Unthinkingly, you open your mouth and scream");
	...
	flags[URTA_SCYLLA_BIG_DICK_TIMES_DONE]++;
...
function watchTwoHotBitchesAndJerkIt():void {
	...
	outputText("Scylla looks shocked, as if she's seeing you for the first time, but her nose twitches and she starts to shovel your seed into her mouth with greedy abandon.  Even so, her body rapidly 'digests' the fluid intake, and the nun starts to slim up before your eyes.  Urta wobbles up onto shaky legs and stumbles into you, giving you a wet kiss that tastes faintly of your salty leavings and then passionately hugging you.  She breaks the embrace and slurs, ", false);
	if(flags[143] == 1) outputText("\"<i>We need to do thish again!</i>\"", false);
...
function makeUrtaSitOnYourLapWithScylla():void {
	...
	if(player.lowerBody == 3) outputText(", snaking your tail around her legs", false);
	else if(player.lowerBody == 8) outputText(", enveloping her legs with your goo", false);
	...
	else if(flags[12] < 11) outputText("a nervous smile on her face, but gives you a quick peck all the same.", false);
	...
	if(flags[143] <= 1) outputText("pay for the door and ", false);
...
function amilyXUrtaUrtaFallout(): void {
	...
	simpleChoices("It's Okay",3201,"Heartbreak",3202,"",0,"",0,"",0);
}
...
function urtaFertilityDiscussion():void {
	...
	if(flags[URTA_FERTILE] == 1) outputText("  Should I stay fertile?");
	...
	if(flags[URTA_PREG_EVERYBODY] == 1) outputText("  Or did you come to ask me not to be fertile and virile around anyone else?");
		...
		if(flags[69] > 0) outputText("  After all, you already had a turn!  Fair's fair!");
	}
	outputText("</i>\"  She winks at you before blowing you a playful raspberry.\n");
	menu();
	if(flags[URTA_FERTILE] == 1) {
...
function urtaPregOthersToggle():void {
	...
	if(flags[URTA_PREG_EVERYBODY] == 1) {
		...
		outputText("Urta smiles salaciously, like a cat that got the creme.  Then, without warning, she kisses you square on the lips, passionately frenching you.  After nearly a full minute of frenzied kissing, she says, \"<i>This is gonna be fun!</i>\"");
		flags[URTA_PREG_EVERYBODY] = 1;
	}
	menu();
	addButton(0,"Back",urtaFertilityDiscussion);
}
...
function urtaDiscussAlcoholDrinkMore():void {
	...
	flags[146] = 1;
	flags[DISCUSSED_URTA_ALCOHOLISM] = 1;
	doNext(13);
}
//[=Drink Less=]
...
function urtaDiscussesEdrynRomance():void {
	...
	if(!urtaLove()) {
		if(!urtaOpenAboutEdryn()) {
			...
			if(player.hasVagina()) outputText("And until you, she was the only woman who didn't freak out and insist it was handjobs or nothing when she saw how big I was.  ");
		...
		//Subsequent Regular Variant:
			...
			outputText("You give Urta thoughtful look, and then ask how things are with Edryn these days.");
			...
			if(player.statusAffectv1("Edryn") >= 5) {
				...
				if(flags[69] > 0) outputText("  Still, I can't believe you actually knocked her up - I had to pick my jaw up off the floor when she told me that the foal kicking inside her ballooning gut was yours.  I mean, those herbs are supposed to be foolproof...</i>\" She shakes her head, half in disbelief of your sheer virility, half in awe of it.");
				else outputText("</i>\"");
			...
			outputText("\n\n\"<i>It's alright... uh, [name]?</i>\" Urta asks, as you stand up, causing you to turn back to face her.  As you wait for an answer, she shakes her head. \"<i>No, nothing, nevermind, forget I said anything.</i>\"");
...
function urtaDiscussesEdrynLoveTolerate():void {
	...
	//{Choosing this option should probably disable both the Romance? discussion topic and the ability to have threesomes with Urta & Edryn - if you can't accept that she needs to fuck mare-cunt when you're not around, then you're probably not the sort of guy who's going to take kindly to the idea of doing her from behind even as she pounds Edryn.}
 	flags[PC_DISLIKES_URTA_AND_EDRYN_TOGETHER] = 1;
	doNext(13);
}
//[=Breakup=]
...
function spankTheShitOutOfUrtaAndMakeHerCreamHerselfFromProstateStimulationAloneLikeTheHornyDrunkenSlutSheReallyIs():void {
	...
	//{PC Don't Got the Dick but Do Got a Tamani Dildo Which is Almost as Good!}
		...
		outputText("\n\nYou swing yourself into Urta's seat, dragging the half-naked fox-girl onto your lap, propping her up with her belly flat across your [legs].  You yank off what remains of her dress, exposing her bare ass to the crowd of patrons.  With gusto, you give her a succession of swats on her defenceless rear, making the drunken fox squeal and squirm in your firm grasp, until you have to restrain her throbbing prick with your [legs].  You give her a little squeeze, making sure she knows just who's in charge here.");
...
//Eggs Option
function giveTheFoxSomeEggs():void {
	clearOutput();
	if(flags[URTA_TIMES_EGG_PREGGED] > 0) {
		repeatUrtaEgging();
		return;
	}
	...
	if(flags[23] > 0) outputText("as much fun as it might be, ");
	...
	if(player.hasPerk("Spider Ovipositor") >= 0) outputText("green slime");
	...
	if(player.hasPerk("Bee Ovipositor") >= 0) outputText("honey");
	...
	flags[URTA_EGGS] = player.eggs();
	flags[URTA_FERTILE_EGGS] = player.fertilizedEggs();
	flags[URTA_TIMES_EGG_PREGGED]++;
	flags[URTA_EGG_INCUBATION] = 72;
	flags[URTA_FLATBELLY_NOTICE] = 0;
	//First time, tag for triggering freakout!
	if(flags[URTA_EGG_FORCE_EVENT] == 0) flags[URTA_EGG_FORCE_EVENT] = 48;
	player.dumpEggs();
	stats(0,0,0,0,0,0,-100,0);
	doNext(13);
}
...
function urtaChewsOutPC(newScreen:Boolean = true):void {
...
function condomlessUrtaInHouseSmex():void {
	clearOutput();
	outputText("You grab hold of the pre-filled bubble atop the fox's tip and yank, earning a moan as the latex sheath slides free, stretching it to an oblong before freeing her flare with an audible pop.  At the same time, you draw the second, unused condom from her unresisting grip and say, \"<i>Why don't we make a little mess?</i>\"");
	//non lover:  
	if(!urtaLove()) outputText("\n\nUrta grumbles something about, \"<i>...can clean up the mess then,</i>\" and sulks, but a quick touch to slippery equine flesh between her legs quiets her complaints.  She looks up at you quizzically, baffled as to just how much of a mess you plan to make.");
	//lover: 
	else outputText("\n\nUrta rolls her eyes and asks, \"<i>Fine, but you'll help me clean up the mess, love.</i>\"  She's practically panting as she says so, cock jutting in the air as ribbons of pre dangle from that rigid tower.  Watching through lusty, half-hidden eyes, the vixen seems curious as to just how messy you want to get.");
...
function pegUrtasAssMessy():void {
	...
	else if(flags[12] < 11) outputText("softly begs, \"<i>Be gentle, please,</i>\" while spreading those fishnet-clad legs a little further apart.");
	...
	outputText("  Scooting forward, you edge your " + cockHead(x) + " up to her anus.  Her body heat washes over you as you enjoy the feel of her pucker's folded flesh pressing down on your most sensitive point, lubricated by the copious female fluid that drains from the sodden gash above.");
	outputText("\n\nYou begin to gently rock your [hips] forward, giving little, gentle pushes against the resistant muscle.  At first, you make no progress, simply forcing her anal ring to dilate, little by little.  Then, after a particularly vigorous stroke, your " + cockHead(x) + " slides through the clinging ring and into the tight embrace of her slick anus.  You stop to allow her to adjust, admiring her ");
	...
	if(flags[12] > 15) outputText("casually");
	else if(flags[12] > 11) outputText("confidently");
	else if(flags[12] > 4) outputText("a little nervously");
	...
	outputText("\n\nYou pinch the nipple you've been playing with and slam your " + cockDescript(x) + " home with rump-shaking force.  It slides the fox backward in her puddle of pre, slicking the underside of her legs with her sexual moisture as she's dragged through it.  Barely suppressing a moan of your own, you shimmy forward again and voraciously tackle her asshole, pegging Urta with short, brutal strokes.  Eager to see her blow, you shift your angle and press up, slamming your " + cockHead(x) + " into her prostate so hard you swear you must be flattening it.  Urta screams, her balls contracting powerfully.  Her dick, already iron-hard, seems to swell thicker, looking like a flat-tipped, over-inflated sausage.  Bulging obscenely, the underside of her cock swells with lusty intent, the distention moving up with muscular contractions until the vulpine girl's cum-slit is wide and filled with white.");
	...
	if(flags[URTA_QUEST_STATUS] < 0) outputText("not-so-fertile ");
	...
	outputText(", kissing your " + cockHead(x) + " every time it peeks past her tits, relishing the taste of her own lust all over it.");
...
function fillMeUpPleaseUrta():void {
	...
	if(player.tongueType > 0) outputText("prodigious ");
	...
	if(flags[23] > 0) outputText("  An exhibitionist thrill runs through you at that, making you even wetter.");
	...
	flags[11]++;
	slimeFeed();
	if(flags[URTA_FERTILE] == 1) player.knockUp(21,515,25);
	stats(0,0,0,0,.2,-3,-100,0);
}
...
function lippleBondUrtaScylla():void {
	...
	outputText("\n\nUrta starts to pant and almost howls as she recognizes Scylla's touch on her cock, travelling towards her swollen, cum-packed balls, probably to attempt to increase the nun's incoming meal.");
	...
	//[If Cock type = Canine]
	if(player.hasKnot(player.biggestCockIndex())) outputText("  You're not making the problem any easier, in fact, your knot halfway stuck inside her breast, surged and filled to full size right after your orgasm.");
...
function nagaOnUrtaMess():void {
	...
	//[if (cocks = 0)]
	if(player.cockTotal() == 0) outputText("\n\nYou can feel her tongue sliding all around and pushing it's way inside your [vagina], pleasure slowly spreading in your body as well as your hermaphrodite vixen girlfriend goes on to lick and slurp on your womanhood, pleasuring you further and further.");
	//[if (cocks > 0)]
	...
	//[if (cocks > 0)
	if(player.cockTotal() > 0) outputText("\n\nUrta's manual and oral stimulation of your maleness is bringing the desired effect as you drip more and more precum into her loving, thirsty maw.  You can decidedly get off on this...");
...
function slurpFawkesCocksForFunAndInflation():void {
	clearOutput();
...
function urtaAndRaphaelSurprise():void {
	clearOutput();
...
function raphaelAndUrtaPartTwo():void {
	...
	if(player.tailType > 0) outputText("lift your tails and ");
...
function urtaGooTesticleVoreRuinedOrgasms():void {
	...
	outputText("\n\nYou chuckle to yourself.  If she likes that, you wonder, how far can you go?  You enlarge and deepen the plug, while having your goo-hands pulse around Urta's cock, as though she were fucking an onna-hole.  The fox writhes under you, biting her lip and whispering inscrutable words.  You allow more of your goo entry, pouring in several inches of it, delighting in the tight feeling you get.  You squeeze her shaft and rub, amazed you can feel the pressure even from the inside.  You detach your hands from one another, letting one remain as a cilia-laden tube stroking up and down the length of Urta's horsey-member.  The other returns to normal, with your index finger lodged right in Urta's pee-hole.");
	...
	outputText("\n\nThis sudden motion causes Urta to cry out in orgasmic ecstasy a third time.  Yet another ruined orgasm laps at your fingers before sucking you deeper.  Your fluidic body gives little resistance, and instead of simply stopping at your shoulder like the laws of physics would dictate, your body begins to reshape, getting pulled deeper into the cock.  You feel a little like a piece of jelly getting sucked through a straw, which suddenly seems incredibly accurate.  Your vision goes dark as a fourth ruined orgasm takes your head.  Deeper and deeper, your body getting hotter and wetter as you go.");
...
function urtasRuinedOrgasmsFromGooPartII():void {
	...
	if(flags[68] == 0) outputText("set of hoofsteps");
	...
	if(flags[68] == 0) outputText("recognize as Edryn's");
	else {
		if(flags[MET_OLIVIA] == 0) outputText("recognize as Olivia's");
	...
	if(flags[68] == 0) outputText("Edryn");
	...
	if(flags[68] == 0) outputText("All right, but I'm still charging you for this one");
	...
	if(flags[68] == 0) outputText("Edryn moan in pleasure");
	...
	outputText(".  Your entire, gooey body feels like one giant clitoris, so sensitive, so vulnerable to pleasure.  And with every slap, your body begins to crave release even more.  The pleasure builds with every thrust and accompanying slap.  It grows to the brink of orgasm, and surges forward, with no end in sight.");
	...
	if(flags[68] == 0) outputText("All you need to do is push, right?");
	...
	if(flags[68] > 0) outputText(", but obliges, sitting back on something, perhaps a bed.  You feel yourself sloshing around oddly as she lies on her back");
	...
	if(flags[68] == 0) outputText("Edryn clutching Urta's hand");
	...
	if(flags[MET_OLIVIA] == 0 && flags[68] != 0) {
...
function wearZeCollar():void {
	...
	//Second Time
	else if(flags[URTA_PETPLAY_DONE] == 1) {
		...
		if(player.tailType > 0) outputText(", tail wagging");
	...
	if(player.tailType == 0) {
		...
		if(player.tailType == 0 && player.ass.analWetness > 0) outputText("  Despite your embarrassment, or perhaps because of it, you find your asshole getting wet with desire, practically salivating in anticipation of that thick plug.");
...
function doggyStyle():void {
	...
	if(player.tailType > 0) outputText("asshole");
	...
	if(player.tailType == 0) outputText(", while she grasps the plug in your ass firmly and pulls. It comes out easily with a wet <b>pop</b>, making your ass feel suddenly empty.  You guess it won't be empty for long, however");
