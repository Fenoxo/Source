...
function teachHelToSwim():void {
	...
	flags[HEL_CAN_SWIM] = 1;
	doNext(13);
}


//Threesomes
function heliaThreesomes():void {
	clearOutput();
...
//needs wings or 50+ str, dick, added as an option \"<i>Carry her to camp</i>\" when you defeat Hel while she is in Fuckbuddy mode.
	...
	if((player.armorName == "goo armor" || flags[VALARIA_AT_CAMP] == 1) && player.lust >= 33 && player.hasVagina()) {
...
function heliaAnalTrainingPrompt():void {
	clearOutput();
	flags[HELIA_ANAL_TRAINING_OFFERED] = 1;
...
function giveHeliaAnalTraining():void {
	...
	outputText("\n\nThe proud salamander berserker explains with a wink, “<i>That won’t make my asshole any looser that it is right now.</i>” She strips naked with aplomb, shaking her hiney in your direction while her tail snuggles around your neck, sinfully warm but less hot than appearances would indicate. A large, knotted buttplug appears, in one of her hands, fitted with a tube that dangles down and connects to a bulb. Helia defly attaches the bottle of booze to the sexual contraption, causing the alcoholic beverage to trickle out a tiny hole in the tip. The device shines wetly in the sun as she pumps a few times to get it completely coated.");
	...
	outputText(", admiring the hefty weight slowly and imagining it violating her stretchy sphincter in the same was as her toy. She grows and lashes her tail around your [leg] to drag you closer, impatiently stabbing four fingers into her twat at a time with such force that her juices occassionally splatter out onto the dirt and you, now that you’ve come close. She helps to guide your [cock biggest] onto her face with her free hand, loudly moaning into it as her hips spread wider.");
	...
	outputText("\n\n“<i>That’s what I’m talking about!</i>” she cheers as she stands up, her legs looking an inch or two farther apart than normal as she rises. “<i>I’m gonna leave this guy in another half our or so to make sure this takes, ");
	//increase Helia capacity.
	flags[HELIA_ANAL_TRAINING] = 1;
...
function heliaGapeSceneChoices():void {
	if(flags[HELIA_ANAL_TRAINING] == 0) giveHeliaAnalTraining();
	else if(flags[HELIA_ANAL_TRAINING] == 1) heliaAnalTrainingPartTwo();
}
...
function heliasBirthday():void {
	...
	flags[HELIA_BIRTHDAY_OFFERED] = 1;
	menu();
...
function helPartyMenu():void {
	menu();
	//heliaDrinks
	if(flags[HELIA_BDAY_DRINKS] == 0) addButton(0,"Drinks",heliaDrinks);
	//heliaHakonAndKiri
	if(flags[HELIA_BDAY_HAKON_AND_KIRI] == 0) addButton(1,"Hakon 'n Kiri",heliaHakonAndKiri);
	//heliaPhoenixes
	if(flags[HELIA_BDAY_PHOENIXES] == 0) addButton(2,"Phoenixes",heliaPhoenixes);
	//heliaAndFoxTwins
	if(flags[HELIA_BDAY_FOX_TWINS] == 0) addButton(3,"Fox Twins",heliaAndFoxTwins);
	//leaveWithoutSex
	addButton(4,"Leave",leaveWithoutSex);
	//leaveWithGirls
	if(flags[HELIA_BDAY_FOX_TWINS] > 0) addButton(5,"Leave w/Girls",leaveWithGirls);
}
...
function goWithHelia():void {
	...
	if(isabellaFollower() || followerKiha() || flags[VALARIA_AT_CAMP] == 1) {
	...
	//If PC did UrtaQuest: 
	outputText("\n\n");
	if(flags[URTA_MET_HEL] == 1) outputText("From the quick look she and the salamander exchange, you get the feeling they've met before. Weird. Still, ");
	...
	if(flags[HARPY_QUEEN_EXECUTED] > 0) outputText("who forgave us for the queen's death");
	...
	if(flags[HEL_FOXY_FOURSOME_WARNED] == 1) outputText("a pair of lovely fox-eared herms");
...
function heliaHakonAndKiri():void {
	...
	flags[HELIA_BDAY_HAKON_AND_KIRI] = 1;
	helPartyMenu();
}
...
function heliaPhoenixes():void {
	...
	if(flags[HARPY_QUEEN_EXECUTED] > 0) outputText("dead");
...
function heliaAndFoxTwins():void {
	...
	if(flags[HEL_FOXY_FOURSOME_WARNED] == 1) outputText("Mai");
