...
function maleFuckCeraphsPussy():void {
	flags[291]++;
	spriteSelect(7);
	outputText("", true);
	...
	if(player.cockArea(0) <= monster.vaginalCapacity()) {
		...
		else if(player.cumQ() >= 1000) outputText("  Her belly rises quickly from the volume of cum you're pumping into her, swelling up until she looks ready to give birth, but her taut purple body seems handle the strain quite well, save for her belly-button turning into an outtie with an abrupt 'pop'.", false);
		...
		else outputText("You bend over and rub her belly softly and whisper into her ear, \"<i><i>I'll be sure to spit him on my cock first chance I get, dear.</i></i>\"\n\n", false);
		...
		else outputText("You bend over and rub her belly softly and whisper into her ear, \"<i><i>I'll be sure to spit them on my cock first chance I get, dear.</i></i>\"\n\n", false);
...
function rideCeraphsCockLikeaBAWSSexclamation11eleven():void {
	flags[291]++;
	slimeFeed();
	spriteSelect(7);
	outputText("", true);
	...
	if(flags[23] >= 1) outputText("getting an exhibitionist thrill ", false);
...
function ceraphSpecial1():void {
	...
	if(monster.hasStatusAffect("Uber") < 0) {
		...
		//(Next Round)
		if(monster.statusAffectv1("Uber") == 0) {
			monster.addStatusValue("Uber",1,1);
...
function ceraphSpecial2():void {
	...
	//[SPECIAL WHILE PC RESTRAINED]
		...
		if(rand(2) == 0) {
			...
			stats(0,0,0,0,0,0,9+player.sens/10,0);
		}
...
//(Struggle)
function ceraphBindingStruggle():void {
	outputText("", true);
...
function ceraphBoundWait():void {
	...
	stats(0,0,0,0,0,0,player.lib/20 + 5 + rand(5),0);
	outputText("\n\n", false);
	enemyAI();
}
...
function ceraphSpecial3():void {
	...
	doNext(1);
	//Blind dodge change
	if(monster.hasStatusAffect("Blind") >= 0 && rand(10) != 9) {
		outputText(monster.capitalA + monster.short + " completely misses you with a blind attack!", false);
	}
	//Determine if dodged!
	...
	//Determine if evaded
		...
		outputText("With Raphael's teachings and the easy movement afforded by your bodysuit, you easily anticipate and sidestep " + monster.a + monster.short + "'s attack.", false);
	}
	//Determine damage - str modified by enemy toughness!
		...
		if(damage <= 0) {
			...
			if(rand(player.armorDef + player.tou) < player.armorDef) outputText("Your " + player.armorName + " absorb and deflect every " + monster.weaponVerb + " from " + monster.a + monster.short + ".", false);
		...
		if(damage > 0 && damage < 6) {
			outputText("You are struck a glancing blow by " + monster.a + monster.short + "! (" + damage + ")", false);
		}
		if(damage > 5 && damage < 11) {
			outputText(monster.capitalA + monster.short + " wounds you! (" + damage + ")", false);
		}
		if(damage > 10 && damage < 21) {
			outputText(monster.capitalA + monster.short + " staggers you with the force of " + monster.pronoun3 + " " + monster.weaponVerb + "! (" + damage + ")", false);
		}
		if(damage > 20) {
			outputText(monster.capitalA + monster.short + " <b>mutilates</b> you with " + monster.pronoun3 + " powerful " + monster.weaponVerb + "! (" + damage + ")", false);
		}
	}
	statScreenRefresh();
	outputText("\n", false);
...
function loseFUCKME():void {
	//Reset rape count if lost
	flags[291] = 0;
	spriteSelect(7);
	if(flags[23] == 0) eventParser(2320);
	else if(flags[23] == 1) eventParser(2321);
	else if(flags[23] == 2) eventParser(2322);
	else {
		ceraphRapesYouBADDAWGYODIGGITY();
	}
}

function ceraphRapesYouBADDAWGYODIGGITY():void {
	slimeFeed();
	spriteSelect(7);
	outputText("", true);
	if((gameState < 1 || gameState > 2) && flags[23] < 3) outputText("The demoness touches your forehead, and suddenly your mind is awash with fetishes!  Judging by the playful look she has, it's temporary, and you play along.  ", false);
	...
	if(player.biggestTitSize() > 1) outputText(", right between your " + allBreastsDescript(), false);
...
function hugeCorruptionForceFuckCeraph():void {
	...
	if(flags[23] > 0) outputText("I'll remove some of my magic from you if you want, and I'll even let you keep the piercing.  Would you like that?", false);
...
function ceraphsNiceOffer():void {
	...
	if(flags[23] > 0) {
		...
		if(flags[23] == 1) outputText("your exhibitionism is gone!", false);
		else if(flags[23] == 2) outputText("your bondage fetish is gone!", false);
...
function friendlyNeighborhoodSpiderManCeraph():void {
	outputText("", true);
	spriteSelect(7);
...
function runFromCeraphsNiceDeal():void {
	...
	stats(0,0,0,0,0,0,(10 + flags[23] * 10),0);
	doNext(13);
}
...
function tradeCeraphSomething():void {
	outputText("", true);
	spriteSelect(7);
	var liqueur:Number = 0;
	var endowment:Number = 0;
	var armor:Number = 0;
	...
	if(hasItem("BimboLq",1)) {
		...
		if(flags[23] > 1) outputText("all the fetishes", false);
		...
		liqueur = 2871;
		if(player.gender > 0 || player.biggestTitSize() > 2) endowment = 2872;
	}
	//(No Bimbo Liquer) 
		...
		if(flags[23] == 1) outputText("your fetish for you.</i>\"\n\n", false);
...
function giveUpYourBallsOrWhateverForLessFetishes():void {
	...
	//(Tits) 
		...
		if(player.breastRows[x].fuckable) flags[217] = 4;
		else if(player.lactationQ() >= 100) flags[217] = 5;
		else flags[217] = 3;
		...
		if(flags[217] == 3) outputText("rolls the nipples in her hands", false);
		else if(flags[217] == 4) outputText("pushes her fingertips inside the leaky nipple-cunts", false);
...
//*Clit pump
//*Orgy floor Drain
function ceraphBodyPartDreams():void {
	spriteSelect(7);
	//Create array for choosing bits!
	var choices:Array = new Array();
	var choice:Number = 0;
	...
	if(choice == 2) {
		...
		if(subChoice == 0) {
			...
			stats(0,0,0,0,0,0,5+player.lib/5,0);
		}
		//Milk Dispenser
		else if(subChoice == 1) {
		...
		//Tit Volleyball
			...
			outputText("You see Ceraph, the conniving omnibus, standing in a hazy pink void, dressed in a tiny purple bikini.  She smiles to herself as she holds out a fleshy orb.  It looks soft, jiggly... and alarmingly familiar.  As you spy a rounded areola, you realise the demon is holding one of your old breasts! She tosses the plump tit ponderously for a moment, before raising it to her mouth and... blowing?");
			outputText("\n\nAs the seductress blows into the nipple, the breast amazingly, impossibly, begins to inflate like a balloon.  She eventually stops and examines her surreal handiwork, your ex-tit now the size of a beachball.  As she bats it into the air and it begins to lazily float back into her hands, you realise it has the same density as a beachball too.");
	...
	//COCKUUUU
		...
		//Cock Sandwich
			...
			outputText("\n\nGushing girl-goo soaks you to the balls as one of the two reaches her peak, her moisture spraying in wild, pleasurable release.  The stud that pierces her hard clit bumps against tip as she thrashes, and you find yourself desperately wishing you were not so restrained by their legs.  If only you could properly thrust, these tainted tramps would be soaked with your cum!  Instead, the best you can do is wiggle from side to side a little bit, which only makes the gift Ceraph granted you all the harder.  Curse whoever she got this dick from!  The hellishly pleasurable sensations build relentlessly, and yet you have no release.");
			...
			stats(0,0,0,0,0,0,5+player.lib/5,.5);
			player.hoursSinceCum += 100;
		}
	}
}

function buttRapeCeraph():void {
	flags[291]++;
	spriteSelect(7);
